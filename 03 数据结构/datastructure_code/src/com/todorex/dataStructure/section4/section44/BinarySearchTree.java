package com.todorex.dataStructure.section4.section44;
...
public class BinarySearchTree<T extends Comparable>  implements Tree<T> {
   ...
   //=================================================================================
    ...
    private BinaryNode<T> findMin(BinaryNode<T> p){
        ...
        if (p==null)//结束条件 终止条件 找不到
            return null;
        else if (p.left==null)//如果没有左结点,那么t就是最小的  终止条件 找到
            return p;
        return findMin(p.left); //递进条件
    }
    ...
    private BinaryNode<T> findMax(BinaryNode<T> p){
        if (p==null)// 递归终止 结束条件
            return null;
        else if (p.right==null) // 递归找到
            return p;
        return findMax(p.right); //递进条件
    }
