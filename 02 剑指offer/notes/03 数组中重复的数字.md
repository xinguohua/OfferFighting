...
我们把从 1 到 n 的数字从中间的数字 m 分为两步，前面一半为 1 到 m ，后面一半为 m+1 到 n 。如果 1 到 n 的数字的数目超过 m ，那么这一半里面一定包含重复的数字；否则另一半 m+1 到 n 的区间里一定包含重复的数字。我们可以继续把包含重复数字的区间一分为二，直到找到一个重复的数字。这个过程和 **二分查找算法** 很类似，只是多了异步统计区间数字的数目。
